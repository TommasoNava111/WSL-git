  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        // === TAIL CORRECTION per energia di Lennard-Jones ===
        ...
        index_property++;
        // === TAIL CORRECTION per pressione di Lennard-Jones ===
        _ptail = 32*M_PI*_rho*(1/(9*pow(_r_cut,9))-1/(6*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
        // GOFR: abilito misura g(r), leggo numero di bin
...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
   // Se qualche misura di penergy, pressure o gofr è attiva, ciclo sulle coppie
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        // Calcolo del vettore distanza con pbc
        distance(0) = this->pbc( _particle(i).getposition(0,true) - _particle(j).getposition(0,true), 0);
        distance(1) = this->pbc( _particle(i).getposition(1,true) - _particle(j).getposition(1,true), 1);
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        // CALCOLO g(r) per coppia (solo se _measure_gofr = true e dr < half-box)
        if (_measure_gofr && dr < _halfside(0)) {
          int bin = static_cast<int>(dr / _bin_size);
          if (bin < _n_bins) {
             // Ogni coppia contribuisce "2" (i->j e j->i) al conteggio
              _measurement(_index_gofr + bin) += 2.0; // 2 per coppia (i,j)
          }
      }
      // Solo se dr < r_cut, accumulo contributo potenziale e viriale
        if(dr < _r_cut){
          // Energia LJ non normalizzata su N (sarà poi divisa + sommata a _vtail)
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          // Contributo viriale per pressione
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
   if (_measure_kenergy) {
    if (_sim_type == 1) {
      // Monte Carlo NVT: usa la temperatura misurata nel passo precedente (_temp)
      kenergy_temp = (3.0/2.0) * _temp;
    } else {
      // Molecular Dynamics: calcola da velocità
      for (int i = 0; i < _npart; i++) {
  ...
  // qui metto metto if che se è quello montecarlo è  3/2 la temperatura
  ...
  } //nel caso montecarlo al posto di kenergy metto 3/2 temp
  // TEMPERATURE ///////////////////////////////////////////////////////////////
...
void System :: averages(int blk){
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  // OUTPUT g(r): sia blocco corrente sia media finale
  if (_measure_gofr) {
    // 1) Salvo il g(r) del blocco corrente: "../OUTPUT/gofr_block_<blk>.dat"
    string fname_blk = "../OUTPUT/gofr_block_" + to_string(blk) + ".dat";
    coutf.open(fname_blk);
    coutf << "# DISTANCE    GOFR_BLOCK" << endl;
    for (int bin = 0; bin < _n_bins; ++bin) {
      double r = (bin + 0.5) * _bin_size;   // punto centrale del bin
      double count_blk = _block_av(_index_gofr + bin) / double(_nsteps); // contatore medio per bin nel blocco
      double norm = (_npart * (_npart - 1)) * 4.0 * M_PI * r * r * _bin_size / _volume;
       // g(r) = ( count / norm ) 
      coutf << setw(12) << r
             << setw(12) << (count_blk / norm) << endl;
    }
    coutf.close();

    // 2) Salvo il g(r) medio finale + incertezza: "../OUTPUT/gofr.dat"
    coutf.open("../OUTPUT/gofr.dat");
    coutf << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
    for (int bin = 0; bin < _n_bins; ++bin) {
      double sum_avg = _global_av(_index_gofr + bin) / _nblocks;  // media su blocchi
      double sum_avg2 = _global_av2(_index_gofr + bin) / _nblocks; // media dei quadri su blocchi
      double err = error(sum_avg * _nblocks, sum_avg2 * _nblocks, _nblocks);
      double r = (bin + 0.5) * _bin_size;
      double norm = (_npart * (_npart - 1)) * 4.0 * M_PI * r * r * _bin_size / _volume;
      double gofr = sum_avg / norm; // valore medio corretto
      double gofr_err = err / norm;  // incertezza normalizzata
      coutf << setw(12) << r
             << setw(12) << gofr
             << setw(12) << gofr_err << endl;
    }
    coutf.close();
  }
