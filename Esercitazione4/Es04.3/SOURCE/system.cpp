...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    cinf.close();
        ...
        vec sumv(_ndim);
        sumv.zeros();
        double vT = sqrt(3.0 * _temp); // Calculate vT from temperature
    
        // Assign velocities along x, y, z cyclically with alternating signs
        for(int i=0; i<_npart; i++) { 
            int axis = i % 3; // Determine axis: 0=x, 1=y, 2=z
            int cycle = i / 3; // Determine cycle number
            double sign = (cycle % 2 == 0) ? 1.0 : -1.0; // Alternate sign every cycle
    
            vx(i) = (axis == 0) ? sign * vT : 0.0;
            vy(i) = (axis == 1) ? sign * vT : 0.0;
            vz(i) = (axis == 2) ? sign * vT : 0.0;
        ...
        // Subtract average velocity for each component to ensure zero total momentum
        for(int idim=0; idim<_ndim; idim++) sumv(idim) /= double(_npart);
        for(int i=0; i<_npart; i++) {
            vx(i) -= sumv(0);
            vy(i) -= sumv(1);
            vz(i) -= sumv(2);
        ...
        // Calculate current kinetic energy and scale to match desired temperature
        double sumv2 = 0.0;
        for(int i=0; i<_npart; i++) sumv2 += vx(i)*vx(i) + vy(i)*vy(i) + vz(i)*vz(i);
        sumv2 /= double(_npart);
        ...
        // Calculate old positions for Verlet integration
        for(int i=0; i<_npart; i++){
    ...
    vec sumv(_ndim);
    sumv.zeros();
    // 1. Assegnazione velocità delta (±vT lungo x)
    double vT = sqrt(3.0 * _temp); // Calcola vT* da T*
    for(int i=0; i<_npart; i++) { //devi fare la stessa cosa su y e z ovviamente quando la metti su ad esempio y gli latri due sono zero
      // Alterna +vT e -vT lungo x
      vx(i) = (i % 2 == 0) ? vT : -vT; 
      vy(i) = 0.0;
      vz(i) = 0.0;
      sumv(0) += vx(i);
    ...
    // 3. Imposta direttamente le velocità SENZA scaling
    for(int i=0; i<_npart; i++) {
    ...
    // 4. Calcola posizioni vecchie per Verlet (codice originale invariato)
    for(int i=0; i<_npart; i++){
      xold = this->pbc( _particle(i).getposition(0,true) - _particle(i).getvelocity(0)*_delta, 0);
      yold = this->pbc( _particle(i).getposition(1,true) - _particle(i).getvelocity(1)*_delta, 1);
      zold = this->pbc( _particle(i).getposition(2,true) - _particle(i).getvelocity(2)*_delta, 2);
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        double vmax=4.0*_temp;
        _bin_size_v = vmax/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    cinf >> comment;
    for(int i=0; i<_npart; i++){ //qui moltiplica 0.5
...
void System :: measure(){ // Measure properties
  ...
  // Modificare in measure():
  if(_measure_pofv){
    //RICORDA CHE HAI MODIFICATO IN _BIN_SIZE_V con la radice della temperatura, al posto che solo la temp
    
    particelle=0;
    for(int i = 0; i < _npart; i++){ //fill the histogram of velocity for the current state of the system
      if(_particle(i).getvelocity_module() < 4.0*_temp){ //check if the velocity is below vmax
        particelle ++;
        int index_occurrence_v = floor((_particle(i).getvelocity_module())/_bin_size_v); //è il bin da incrementare nell'istogramma
        index_occurrence_v = _index_pofv + index_occurrence_v; //_index_pofv is the fist cell reserved in _measurement for POFV, _index_pofv + _n_bin_size is the last one
        _measurement(index_occurrence_v)++; //each _measurement's reserved cells is a bin of the histogram
      }
    }
    if(particelle>0){
...
void System :: averages(int blk){
 ...
 // medie sui blocchi e non medie cumulative medie progressive
 if(_measure_pofv && blk == _nblocks){ // stampa solo all'ultimo blocco
  ...
  for(int i = 0; i < _n_bins_v; i++){
    ...
    // La media e l'errore saranno calcolati sui dati già normalizzati:
