...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    cinf.close();
        ...
        vec sumv(_ndim);
        sumv.zeros();
         // Calcolo di vT: la velocità corrispondente alla temperatura di input
        double vT = sqrt(3.0 * _temp); // vT* = sqrt(3 T*) in unità ridotte
    
          // Assegno a ogni particella una velocità ±vT su un solo asse (x, y o z) in modo ciclico
        for(int i=0; i<_npart; i++) { 
            int axis = i % 3; // Determina l'asse: 0→x, 1→y, 2→z
            int cycle = i / 3; // Indice del "ciclo" per alternare il segno
            double sign = (cycle % 2 == 0) ? 1.0 : -1.0;  // Alterno + e – ogni ciclo
             // Imposto solo la componente corrispondente a "axis" a ±vT, le altre a 0
            vx(i) = (axis == 0) ? sign * vT : 0.0;
            vy(i) = (axis == 1) ? sign * vT : 0.0;
            vz(i) = (axis == 2) ? sign * vT : 0.0;
     // Accumulo le velocità per calcolare la derivata di momento totale
            sumv(0) += vx(i);
            sumv(1) += vy(i);
            sumv(2) += vz(i);
        }
    
        // Sottraggo la velocità media per ciascuna componente per avere momento totale zero
        ...
        // Calcolo dell'energia cinetica corrente per ogni particella (prima dello scaling)
        double sumv2 = 0.0;
        for(int i=0; i<_npart; i++) sumv2 += vx(i)*vx(i) + vy(i)*vy(i) + vz(i)*vz(i);
        sumv2 /= double(_npart);
          // Fattore di scala per far corrispondere la KE a (3/2) T*
        double scalef = sqrt(3.0 * _temp / sumv2);
    
        // Applico il fattore di scala e assegno le velocità definitive all'oggetto Particle
        for(int i=0; i<_npart; i++) {
        ...
        // Calcolo delle posizioni "vecchie" per il primo passo Verlet:
        // x_old = x(t) – v * Δt (con condizioni periodiche)
        for(int i=0; i<_npart; i++){
